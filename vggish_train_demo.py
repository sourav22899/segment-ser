...
def _get_examples_batch():
  ...
  _,_,sine_examples = vggish_input.waveform_to_examples(sine, sr)
  sine_labels = np.array([[1, 0, 0]] * sine_examples.shape[0])
  _,_,const_examples = vggish_input.waveform_to_examples(const, sr)
  const_labels = np.array([[0, 1, 0]] * const_examples.shape[0])
  _,_,noise_examples = vggish_input.waveform_to_examples(noise, sr)
